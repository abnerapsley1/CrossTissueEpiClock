...
# Read in iDAT file names and phenotypes from EPIC array 2 #
# Pattern should be the name of the sample sheet #
targets <- read.metharray.sheet(dataDirectory, pattern="SampleSheet_Batch2.csv")
...
saveRDS(ControlBeta, "Batch2_ControlBeta.RDS")
...
write.csv(rownames(detP[!keep,]), "Batch2_pvalFilt_Probes.csv")
# Remove poor quality probes from detection p-value data #
detP <- detP[keep,]
# Remove poor quality probes from beta values #
mSetSqBMIQ <- mSetSqBMIQ[keep,]
...
saveRDS(mSetSqBMIQ, "bVals_SampFilter_ProbeFilt_BMIQNorm_Batch2.RDS")