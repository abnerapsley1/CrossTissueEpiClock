...
# Read in iDAT file names and phenotypes from EPIC array 3 #
# Pattern should be the name of the sample sheet #
targets <- read.metharray.sheet(dataDirectory, pattern="SampleSheet_Batch3.csv")
...
saveRDS(ControlBeta, "Batch3_ControlBeta.RDS")
...
write.csv(rownames(detP[!keep,]), "Batch3_pvalFilt_Probes.csv")
# Remove poor quality probes from detection p-value data #
detP <- detP[keep,]
# Remove poor quality probes from beta values #
mSetSqBMIQ <- mSetSqBMIQ[keep,]
...
saveRDS(mSetSqBMIQ, "bVals_SampFilter_ProbeFilt_BMIQNorm_Batch3.RDS")