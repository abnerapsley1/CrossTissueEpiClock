...
# Read in iDAT file names and phenotypes from EPIC array 4 #
# Pattern should be the name of the sample sheet #
targets <- read.metharray.sheet(dataDirectory, pattern="SampleSheet_Batch4.csv")
...
saveRDS(ControlBeta, "Batch4_ControlBeta.RDS")
...
write.csv(rownames(detP[!keep,]), "Batch4_pvalFilt_Probes.csv")
# Remove poor quality probes from detection p-value data #
detP <- detP[keep,]
# Remove poor quality probes from beta values #
mSetSqBMIQ <- mSetSqBMIQ[keep,]
...
saveRDS(mSetSqBMIQ, "bVals_SampFilter_ProbeFilt_BMIQNorm_Batch4.RDS")